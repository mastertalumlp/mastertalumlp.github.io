...
class Creneau:
	...
	def __str__(self):
		...
		return str(self.semaines) + ", " + self.jour + ", " + str(self.debut) + "h-" + str(self.fin) + "h, " + self.nature + " \"" + code + self.intitule[0:10] + "..." + intvn + "\"" + salle
...
def checkSemestre(semvalue, filterlist):
	semvalues = (str(semvalue)).split(",")
	lst = []
	for f in filterlist:
		lst.append(str(f).strip())
	for s in semvalues:
		if s.strip() in lst: return True
	return False
...
class SousListeCreneaux:
	...
	# par ex. {"semestre": [7,8], "titre": "Master 1 TAL"}, {"semestre": [9,10], "titre": "Master 2 TAL"},
	...
	def __init__(self, lstCr, filter, semaines):
		...
		for c in lstCr :
			if (filter.__contains__("semestre") and filter["semestre"] != ""):
				if not checkSemestre(c.semestre, filter["semestre"]): continue
...
def extractEDTenHTML(sousListe, html_rel_path, fname, titre, semaines, annees, distinction_param="code"):
    ...
    # dictionnaire pour les semaines
    sem_dict = dict()
    ...
    for s in semaines:
        ...
        html_titre = "#### Semaine " + str(s) + " : " + titre + " (" + dateDebut(s, annee) + "-" +  dateFin(s, annee) + ")\n\n"

        html_tab = '<table class="table table-sm table-bordered" style="width:100%; max-width:1200px; min-width:700px;">'
        html_tab += '<tr class="table-active text-center"><th style="width:5%"></th>'

        js_ligne = ""

        for jourNum in  range(5) :
            js_ligne += '<th style="width:19%">'
            js_ligne += joursSemaine[jourNum] + " " + datetime.date.fromisocalendar(annee, s, jourNum+1).strftime("%d/%m")
            js_ligne += '</th>'

        html_tab += js_ligne + "</tr>\n"
        ...
        sem_dict[s] = html_titre + html_tab
    ...
    if (len(sousListe.conflits)) :
        htmlconf = '<div class="col-2 float-right"><div class="card text-danger border-danger"> <div class="card-header fw-bold text-white bg-danger">Conflits de créneaux</div>'
...
# semaines = [6, 7, 9, 10, 11, 12, 13, 14, 15, 21, 25]
semaines = [ 21, 26]
annees = [2025, 2026]
...
df = lire_google_sheet("/home/iana/CLOUDS/travail/10-Espace-de-travail-perso/paroli/credentials/edt-master-tal-27f3c1946ece.json", "Donnees-Master-TAL", "EDT-2025-2026")
#print(df.head())
